...
As mentioned in the section Nested Classes, nested classes enable us to logically group classes that are only used in one place, increase the use of encapsulation, and create more readable and maintainable code. Local classes, anonymous classes, and lambda expressions also impart these advantages; however, they are intended to be used for more specific situations:

**Local class:** 
- Use it if we need to create more than one instance of a class, access its constructor, or introduce a new, named type (because, for example, we need to invoke additional methods later).

**Anonymous class:** 
- Use it if we need to declare fields or additional methods.

**Lambda expression:**
- Use it if we are encapsulating a single unit of behavior that we want to pass to other code. For example, we would use a lambda expression if we want a certain action performed on each element of a collection, when a process is completed, or when a process encounters an error.
